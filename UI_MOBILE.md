...
| `#onboarding-welcome-panel` 新手欢迎 | 无标题栏的小窗；正文保持两行，iOS 风格“不再弹出”开关与并排双按钮不压缩到不可点。 |
| `#deepseek-setup-panel` DeepSeek 配置 | 三步进度和当前步骤正文纵向排列；正文独立滚动，底部主操作始终可见，Key 输入保持至少 16px。 |
...
#### 新手欢迎与 DeepSeek 快速配置专项
- **欢迎窗**：`#onboarding-welcome-panel` 没有 `.otome-panel__head`，横屏规则不能为它补造标题区；正文、开关、双按钮按自然流排列，在极矮视口允许面板正文滚动。
- **三步向导**：`#deepseek-setup-panel` 顶部只保留“获取 Key / 导入 / 检查”三步进度；每次只显示当前步骤。打开官方页后仍留在第 1 步，只有点击“我已复制，下一步”才进入第 2 步。正文区域使用 `min-height:0; overflow-y:auto`，底部按钮区使用 `flex:none`，避免 915×412 下“打开官方页”“读取 Key”“检查配置”“开始游戏/返回游戏”等主操作被挤出屏幕。
- **外开边界**：注册、登录、创建 Key 和充值仍由系统浏览器或新标签打开 DeepSeek 官方页；横屏层只调整游戏内向导，不嵌入、不覆盖或模拟官方页面。
- **输入规则**：剪贴板读取失败后显示的密码输入框必须保持 `font-size:16px`；错误文案可换行，不能通过缩小字体塞进单行。键盘弹出时正文可滚，底部按钮不能与输入框重叠。
- **操作模式**：无论从欢迎窗还是设置页 `#settings-deepseek-quick-setup` 进入，向导显示期间都保持非操作模式；关闭向导后直接返回游戏并恢复操作。横屏切换和浮层互换期间不得短暂显示 HUD/准星或请求 Pointer Lock。
- **样式边界**：横屏专属选择器仅写在 `css/mobile_landscape.css`，并以 `html.ml-active #onboarding-welcome-panel` / `html.ml-active #deepseek-setup-panel` 限定；桌面与移动竖屏分别由 `panels.css` / `responsive.css` 负责。
...
6. 新手引导：清除“不再弹出”偏好后，在 1440×900 桌面、390×844 竖屏和 915×412 `?ml=1` 横屏分别检查欢迎窗；两行文案、开关和双按钮应完整，无标题栏，不遮挡也不溢出。
7. DeepSeek 向导：在 915×412 `?ml=1` 依次打开“获取 Key / 导入 / 检查”，检查三步进度不换成竖排、正文可滚、底部主按钮始终可见；模拟剪贴板拒绝后手动 Key 输入 computed `font-size` 必须至少 16px，软键盘出现时仍可滚到按钮。
8. 系统设置 → 操作设置：鼠标/触控/本地化灵敏度在同一个无标题圆角框内，每行是左标题说明、右上数值、右下滑块，不出现高级设置的类别标题和变量名；
9. 系统设置 → 更多资源：左栏 LOGO + 标题整体居中，右栏更宽；`Copyright` 首行在 915×412 一类横屏宽度下尽量不换行，资源按钮保持 2×2；
10. 战术考核：移动横屏卡牌模式下左上角 `SNOWFIELD / 战术考核` 标题卡片隐藏、右上角返回按钮保留；整体仍是既有横屏布局（手牌居中、人物居中、敌方在右），但**手牌两侧 4 控件（牌库/刷新/弃牌/出牌数）贴在卡牌左右并略留空隙、夹在卡牌与角落按钮之间且互不重叠**；卡池窗口约为桌面 2/3 宽，列表视窗由 JS 按当前实际渲染的 4 张原始高度剩余手牌动态计算，并保留滚动条，不改变卡池条目的高度、padding、gap 或字体；手牌区卡牌更大、标题/描述更小而底部数值不缩小。**若 4 控件离卡牌太近/太远，调 `--ml-fbtn-gap`；若仍撞角按钮，调 `--ml-hand-w` / `--ml-fbtn` / `--ml-fbtn-b1`，不要整体平移**；
11. `?ml=0` 与桌面宽高比对照：浮层与战术考核外观应与现状**完全一致**（无回归）；
12. 真机（Android WebView APP / 手机浏览器）横屏抽测，确认 DeepSeek 官方链接外开后回到游戏仍停留在原向导步骤。
